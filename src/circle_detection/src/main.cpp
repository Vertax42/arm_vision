...
struct Command
{
  ...
  Command(double _x = -0.2, double _y = 0.0, double _yaw = 0.0, double _cycle_time = 0.64, double _move = 0.) : x(_x), y(_y), yaw(_yaw), cycle_time(_cycle_time), move(_move) {}
} user_cmd;
                      ...
                      std::vector<double> &torque)
{
  ...
  // 11 RIGHT_ARM_YAW_JOINT
  // 12 RIGHT_ELBOW_PITCH_JOINT
  // 13 RIGHT_ELBOW_YAW_JOINT
  // 14 RIGHT_WRIST_ROLL_JOINT
  // 15 RIGHT_WRIST_YAW_JOINT
  ...
  // 18~23 left leg(6)
  // 18 LEFT_LEG_ROLL_JOINT
  // 19 LEFT_LEG_YAW_JOINT,
  // 20 LEFT_LEG_PITCH_JOINT,
  // 21 LEFT_KNEE_JOINT,
  // 22 LEFT_ANKLE_PITCH_JOINT,
  // 23 LEFT_ANKLE_ROLL_JOINT,
  
  // 24~29 right leg(6)
  // 24 RIGHT_LEG_ROLL_JOINT
  // 25 RIGHT_LEG_YAW_JOINT,
  // 26 RIGHT_LEG_PITCH_JOINT,
  // 27 RIGHT_KNEE_JOINT,
...
void processKeyboardInput(char ch)
{
  ...
  if (ch == 'w')
  {
    user_cmd.x = std::min(0.6, user_cmd.x + 0.25);
    user_cmd.move = 1.;
  }
  if (ch == 's')
  {
    user_cmd.x = std::max(-0.4, user_cmd.x - 0.2);
    user_cmd.move = 1.;
  }
  if (ch == 'a')
  {
...
int main(int argc, char **argv)
{
  ...
  printf_program("XBot on rl_policy: RL inference for XBot Locomotion");
  common_tools::printf_software_version("XBot_RL");
