...
void BerxelImageRender::drawDepthValue2(uint16_t* pData, uint16_t width, uint16_t height, berxel::BerxelHawkPixelType pixelType, const WinRect& rect) {
	if (NULL == pData)
	{
		return;
	}
	...
	uint32_t curIndex = width * (m_CursorPos.y - rect.y) * ((height * 1.0) / rect.h) + (m_CursorPos.x - rect.x) * ((width * 1.0) / rect.w);

	if (curIndex >= (width * height))
	{
		return;
	}
	//uint16_t* pData = (uint16_t*)pFrame->getData();
